...
# For loop to run the script for each frame (0-299)
for i in {1..2}
do
    rm -rf $workdir/sparse_
    rm -rf $workdir/image_colmap
    python scripts/"$datatype"2colmap.py $workdir $i
    ...
    colmap stereo_fusion --workspace_path $workdir/colmap/dense/workspace --output_path $workdir/fused_$i.ply
done
